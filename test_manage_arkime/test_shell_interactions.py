...
def test_WHEN_call_shell_command_called_AND_cwd_provided_AND_env_provided_THEN_process_spawned_as_expected(
    ...
    # Run our test
    with pytest.raises(EndTestExpectedException):
        shell.call_shell_command('test command', cwd='cwd', env={"MY_VAR": "var value"})

    # Check our results
    expected_calls = [mock.call('test command', cwd='cwd', env={"MY_VAR": "var value"}, timeout=None)]
    assert expected_calls == mock_pexpect.spawn.call_args_list
...
def test_WHEN_call_shell_command_called_THEN_returns_expected_values(mock_pexpect):
    ...
    # Run our test
    actual_exit_code, actual_std_out = shell.call_shell_command('test command', cwd='cwd', env={"MY_VAR": "var value"})
